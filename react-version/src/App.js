...
function App() {
  const [account, setAccount] = useState(accounts[0]) // useState({})
  const { movements=[], owner:user='' } = account
  /* 
   TAREAS
   2. Hacer los movimientos.
      Recibe una propiedad que es el array de los movimientos.
      Muestra una lista de movimientos que son un componente llamado Movement que recibe una propiedad que es el movimiento.
